...
def scale_angle(value, axis): # scaling for the degree of freedom of human head.
    ...
    if axis == "y":
        ...
        return round(degree), round(radian,4)
    if axis == "z":
        degree = value * 360 * 3.5 # idk why 3.5
        radian = degree * np.pi/180
        return round(degree), round(radian,4)
...
def get_head_positions(image, results, angle_type, show_text=True):
    ...
    if results.multi_face_landmarks :
        for face_landmarks in results.multi_face_landmarks :
            ...
            #print(angles) #cv2 returns angles in zyx order

            head_pitch_degree, head_pitch_radian = scale_angle(angles[1],"y")
            head_yaw_degree, head_yaw_radian = scale_angle(angles[0],"z")
            head_roll_degree, head_roll_radian = scale_angle(angles[2],"x")
            ...
            #        cv2.putText(image,"Head Roll: " + str(head_roll_degree) + " Pitch: " + str(head_pitch_degree) + " Yaw: " + str(head_yaw_degree), (image.shape[1]//2-290,100), cv2.FONT_HERSHEY_SIMPLEX , 1 , (255 , 255 , 255) , 2)
            ...
            output["Roll"]["Degree"] = head_roll_degree
            output["Roll"]["Radian"] = head_roll_radian
